...
confirms that JSON parses, expected top-level fields exist, each fixture is
paired with the expected schema file name, and referenced prompt ids look like
`PROMPT_01`. It is not a full JSON Schema validator and does not enforce every
rule in `schemas/*.schema.json`.
